...
const NOW = new Date(),
  ...
  genAll = async (sync) => {
    ...
    for (const [accountKey, hosts] of Object.entries(HOST_HW)) {
      for (const [domain, host_conf] of Object.entries(hosts)) {
        const key_crt = await gen(accountKey, domain, sync);
        if (key_crt) updates.set(domain, { key_crt, host_conf });
      }
    }
    if (updates.size) {
      await runHook();
      if (!sync) await cdn(updates);
    }
  };
